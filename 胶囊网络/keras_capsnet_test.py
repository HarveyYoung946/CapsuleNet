...
def build_models():
    ...
    #解码
    true_labels_input = Input(shape=(output_shape,))
    masked_by_true_label = Mask()([digit_caps,true_labels_input])
    masked_by_max = Mask()(digit_caps)
...
def main():
    ...
    train_model,evl_model = build_models()
    print(train_model.summary())
    train_model.compile(loss=[margin_loss,'mse'],optimizer='adam')
    train_model.fit([x_train,y_train],[y_train,x_train],batch_size=256,epochs=1,validation_data=[[x_test,y_test],[y_test,x_test]])
